...
function cumdist(cumsum, n) {
  ...
  var l = ps[ps.length - 2] || ps[ps.length - 1];  // in case ps is an array of 1
  var r = ps[ps.length - 1];
  var run = r - l;
  var rise = icdf[r] - icdf[l];
  var slope = run === 0 ? 0 : rise / run;
  var newspeed = Math.round(+icdf[r] + slope * (1e6 - r));
  icdf[1e6] = newspeed;
  cdf[newspeed] = 1;
  ...
  // (for faster computation, key of cdf hash is integer; therefore round off values of icdf hash as well)
  l = ps[0];
  r = ps[1] || ps[0];  // in case ps ian an array of 0
  run = r - l;
  rise = icdf[r] - icdf[l];
  slope = run === 0 ? 0 : rise / run;
  newspeed = Math.round(+icdf[l] - slope);
  icdf[0] = newspeed;
  cdf[newspeed] = 0;
